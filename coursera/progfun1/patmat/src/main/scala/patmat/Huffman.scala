...
object Huffman {
  ...
  /**
    ...
    */
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs.map(t => Leaf(t._1, t._2)).sortBy(l => l.weight)
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    if (trees.length < 2) trees
      ...
      (newTree :: trees.tail.tail).sortBy(weight)
    }
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def process(remaining: CodeTree, bits: List[Bit]): List[Char] = remaining match {
      case Leaf(c, w) if bits.isEmpty => List(c)
      case Leaf(c, w) => c :: process(tree, bits)
      case Fork(left, right, c, w) if bits.head == 0 => process(left, bits.tail)
      case Fork(left, right, c, w) => process(right, bits.tail)
    }
    process(tree, bits)
  }
  ...
  /**
    ...
    */
  val frenchCode: CodeTree = Fork(Fork(Fork(Leaf('s', 121895), Fork(Leaf('d', 56269), Fork(Fork(Fork(Leaf('x', 5928), Leaf('j', 8351),
    List('x', 'j'), 14279), Leaf('f', 16351), List('x', 'j', 'f'), 30630), Fork(Fork(Fork(Fork(Leaf('z', 2093), Fork(Leaf('k', 745),
    Leaf('w', 1747), List('k', 'w'), 2492), List('z', 'k', 'w'), 4585), Leaf('y', 4725), List('z', 'k', 'w', 'y'), 9310),
    Leaf('h', 11298), List('z', 'k', 'w', 'y', 'h'), 20608), Leaf('q', 20889), List('z', 'k', 'w', 'y', 'h', 'q'), 41497),
    ...
    Fork(Fork(Fork(Leaf('r', 100500), Fork(Leaf('c', 50003), Fork(Leaf('v', 24975), Fork(Leaf('g', 13288),
      ...
      List('r', 'c', 'v', 'g', 'b'), 202588), Fork(Leaf('n', 108812), Leaf('t', 111103), List('n', 't'), 219915),
      List('r', 'c', 'v', 'g', 'b', 'n', 't'), 422503), Fork(Leaf('e', 225947), Fork(Leaf('i', 115465), Leaf('a', 117110),
      List('i', 'a'), 232575), List('e', 'i', 'a'), 458522), List('r', 'c', 'v', 'g', 'b', 'n', 't', 'e', 'i', 'a'), 881025),
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def process(tree: CodeTree)(char: Char): List[Bit] = tree match {
      case Leaf(c, w) => List.empty
      case Fork(left, right, c, w) if chars(left).contains(char) => 0 :: process(left)(char)
      case Fork(left, right, c, w) => 1 :: process(right)(char)
    }

    text.flatMap(process(tree))
  }
  ...
  def convert(tree: CodeTree): CodeTable = tree match {
    case Leaf(c, w) => List((c, List.empty))
