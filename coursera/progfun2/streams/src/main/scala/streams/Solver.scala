...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    ...
    neighbors.filter { case (b, ms) => !explored.contains(b) }
  }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (b, ms) #:: rest =>
      val neighbors = newNeighborsOnly(neighborsWithHistory(b, ms), explored + b)
      neighbors ++ from(rest ++ neighbors, explored + b)
  }
  ...
  /**
    ...
    */
  lazy val pathsToGoal: Stream[(Block, List[Move])] = pathsFromStart.filter { case (b, ms) => done(b) }
  ...
  lazy val solution: List[Move] = {
    if (pathsToGoal.isEmpty) List()
      ...
      val path = pathsToGoal.minBy { case (b, ms) => ms.size }
      path match {
        case (b, ms) => ms
      }
    }
