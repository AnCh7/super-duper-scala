...
package object barneshut {
  ...
  case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: Seq[Body]) extends Quad {
    ...
    val (massX, massY) = (
      bodies.map(i => i.mass * i.x).sum / mass,
      bodies.map(i => i.mass * i.y).sum / mass)
    ...
    def insert(b: Body): Quad = {
      ...
      if (size > minimumSize)
        (b +: bodies).foldLeft(Fork(
          Empty(centerX - sq, centerY - sq, sd),
          Empty(centerX + sq, centerY - sq, sd),
          Empty(centerX - sq, centerY + sq, sd),
          Empty(centerX + sq, centerY + sq, sd)))(_.insert(_))
      else
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      val x: Int = ((b.x - boundaries.minX) / (boundaries.width / sectorPrecision)).toInt
      val y: Int = ((b.y - boundaries.minY) / (boundaries.height / sectorPrecision)).toInt
      this.apply(x, y) += b
      this
    }
